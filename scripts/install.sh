...
示例:
  ...
  sudo ./scripts/install.sh --host 127.0.0.1 --port 8000 --user zzc
...
if [[ "$(id -u)" -ne 0 ]]; then
  echo "请用 root 运行，例如：sudo ./scripts/install.sh" >&2
  exit 1
fi
...
if [[ ! -f "${APP_DIR}/server/main.py" || ! -f "${APP_DIR}/requirements.txt" ]]; then
  echo "项目目录缺少 server/main.py 或 requirements.txt" >&2
  exit 1
fi
...
if "${PYTHON_BIN}" -m venv "${VENV_DIR}" >/tmp/${SERVICE_NAME}-venv.log 2>&1; then
  echo "==> 使用虚拟环境安装依赖"
  ...
  if run_as_user "${PYTHON_BIN}" -m pip install --user -r "${REQ_FILE}"; then
    true
  else
    echo "==> 普通 --user 安装失败，尝试 Debian/Ubuntu PEP668 兼容参数 --break-system-packages"
    run_as_user "${PYTHON_BIN}" -m pip install --user --break-system-packages -r "${REQ_FILE}"
  fi
