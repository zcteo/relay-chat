...
let questionNavScrollTimer = null;
...
function renderAccountStatus() {
  ...
  $("logoutAccount").classList.toggle(
    "hidden",
    !(isServerMode() && !!username),
  );
...
function renderMessages() {
  ...
  for (let i = 0; i < s.messages.length; i++) {
    box.appendChild(messageEl(s.messages[i], i));
  }
  scrollMessagesToBottom();
  renderQuestionNav();
  updateScrollBottomButton();
}
function messageEl(m, index = 0) {
  ...
  c.className = "content";
  if (m.role === "assistant")
    c.innerHTML = MarkdownLite.render(m.content || "");
...
function questionNavTitle(text, fallback) {
...
function renderQuestionNav() {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const target = box.querySelector(
      `.msg.user[data-message-index="${index}"]`,
    );
    const button = document.createElement("button");
    const title = questionNavTitle(message.content, `问题 ${i + 1}`);
    button.type = "button";
    button.className = "question-nav-item";
    button.dataset.messageIndex = String(index);
    button.setAttribute("aria-label", `跳转到问题 ${i + 1}：${title}`);
    const number = document.createElement("span");
    number.className = "question-nav-number";
    number.textContent = String(i + 1);
    const tip = document.createElement("span");
    tip.className = "question-nav-tip";
    tip.textContent = title;
    button.append(number, tip);
    button.onclick = () => scrollToQuestion(index);
    button.messageTarget = target;
    nav.appendChild(button);
  }
  updateQuestionNav();
}
function messageTopInScrollBox(node, box) {
  return node.getBoundingClientRect().top - box.getBoundingClientRect().top;
}
function updateQuestionNav() {
  const nav = $("questionNav");
  const box = $("messages");
  if (!nav || nav.classList.contains("hidden")) return;
  const buttons = [...nav.querySelectorAll(".question-nav-item")];
  if (!buttons.length) return;
  const anchor = Math.min(180, box.clientHeight * 0.28);
  let active = buttons[0];
  for (const button of buttons) {
    const node = button.messageTarget;
    if (node && messageTopInScrollBox(node, box) <= anchor) active = button;
  }
  for (const button of buttons) {
...
function scrollToQuestion(index) {
  const box = $("messages");
  const node = box.querySelector(`.msg.user[data-message-index="${index}"]`);
  if (!node) return;
  ...
  const top = Math.max(
    0,
    box.scrollTop + messageTopInScrollBox(node, box) - 18,
  );
  box.scrollTo({ top, behavior: "smooth" });
  updateScrollBottomButton();
  window.clearTimeout(questionNavScrollTimer);
  questionNavScrollTimer = window.setTimeout(updateQuestionNav, 240);
}
...
function titleGenerationContext(session) {
  const parts = [];
  let total = 0;
  const messages = session.messages || [];
  for (let i = 0; i < messages.length - 1; i++) {
    ...
    const userContent = String(user.content || "");
    const assistantContent = String(assistant.content || "");
    const userPart = userContent.slice(0, 256);
    const assistantPart = assistantContent.slice(0, 1024);
    parts.push(`用户：${userPart}`, `助手：${assistantPart}`);
    total += userPart.length + assistantPart.length;
    if (total >= 512) break;
    i += 1;
  }
  if (total < 512) return "";
...
async function generateSessionTitle(sessionId) {
  ...
  const titleContext = titleGenerationContext(session);
  ...
  try {
    ...
    if (!title) return;
    if (!titleCanBeAutoGenerated(latest)) return;
    latest.title = title;
...
for (const id of ["theme", "thinking", "maxTokens", "systemPrompt"])
